...
export const NamePage: React.FC<Props> = (props) => {
  ...
  const swrHatenaListFetch = useSWRHatenaDetailFetch(title || '');
  const html = props.ssr.hatenaDetailData
    ? props.ssr.hatenaDetailData
    : swrHatenaListFetch.data;

  return (
    <>
      <NextHead>
        <title>{props.ssr.ogp.title} / Hatena バックアップ / syonet</title>
        <meta name="description" content={props.ssr.ogp.description} />
        <meta
          property="og:title"
          content={`${props.ssr.ogp.title} / Hatena バックアップ / syonet`}
        />
